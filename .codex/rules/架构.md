# 企业商务差旅规划 Agent 项目总 Spec

> 文档状态：Core Frozen v2.0，M0～M6 求职完成线已冻结；M4.1 进入 Plan  
> 文档类型：项目级总规格（Master Specification）  
> 权威级别：本项目业务边界、目标架构、技术路线与全局验收的唯一设计依据  
> 当前实现基线：M0、M1、M2、M2.1、M3、M4 已完成历史验收  
> 当前待实施阶段：M4.1 商务差旅评测基线  
...
项目采用规格驱动、评测驱动和失败优先的开发方法。项目总 Spec 冻结业务目标、技术选型和全局合同；阶段 Spec 定义本阶段增量与验收边界；Implementation Plan 将已批准的阶段 Spec 拆解为测试驱动任务；Offline Suite、Live Gate、代码评审和阶段报告共同决定阶段是否完成。文档中的技术判断是待验证假设，最终结论必须由代码、测试、真实工具实验和量化报告共同支持。
...
2. **技术选型 Spec**：说明模型、确定性服务、工具、数据和基础设施如何分工，以及为什么选择当前方案；
3. **实施蓝图**：规定 M0～M9 的依赖关系、阶段目标和完成线；
...
截至本文版本，M0、M1、M2、M2.1、M3 和 M4 已完成历史验收。已实现 Pydantic 领域模型、状态与错误、Interpreter/Router、请求级可观测性、Tool Port 与 Amap/Tuniu Adapter、Evidence Registry、CandidatePool、Task Graph、Research Agent、Schedule/Budget 和结构化 Composer。

历史验收只证明通用内核在原规格下成立，不证明商务差旅纵向链路已经成立。当前状态必须描述为：

```text
Implemented: M0～M4 通用内核
Specified: M4.1～M9 差旅业务目标
Not implemented: BusinessTripScopeResolver、PolicyKnowledgePort/PDF Adapter、完整市内交通切片、G0～G5 统一闭环、API Demo、SQLite Replan、Action、生产韧性
```
...
- 总 Spec、阶段 Spec、Plan、测试与实验报告组成的规格驱动流程。
...
> 严格实施规格见 `.codex/rules/roadmap/00-总路线图.md` 及 M0～M9 阶段文件，其中包含插入阶段 M2.1、M4.1、M4.2 与 M4.3。M0～M6 是求职项目核心完成线，M7 是进阶差异化能力，M8～M9 是岗位定向扩展。执行任何阶段前必须阅读本总 Spec、总路线图和对应阶段规格，前一阶段未验收不得启动后一阶段。
...
```text
.codex/rules/架构.md                 项目总 Spec：唯一业务和架构依据
.codex/rules/roadmap/00-总路线图.md  阶段依赖与完成线
.codex/rules/roadmap/Mx-*.md         阶段 Spec：本阶段增量合同
阶段 Implementation Plan            文件级任务、测试和执行顺序
代码 / 测试 / Eval / 报告            实验证据
学习笔记与历史材料                   背景，不覆盖当前 Spec
```

项目总 Spec 回答“为什么做、做什么、全局如何判断成功”；阶段 Spec 回答“本阶段改变什么、不改变什么、接口和验收是什么”；Plan 回答“按什么顺序改哪些文件、先写哪些失败测试、如何验证”。Plan 不能替代设计，代码也不能通过既成事实反向篡改规格。

### 23.2 阶段 Spec 的最低内容

每个阶段 Spec 必须包含：背景和问题证据、当前状态、目标状态、非目标、业务规则、输入输出 Schema、模块责任、状态与错误语义、失败路径、安全边界、测试矩阵、Eval 指标、通过门槛、迁移和兼容策略、开放问题及进入下一阶段的条件。

“实现一个酒店 Agent”“优化 Router”“接入 RAG”不是可执行规格。规格必须说明什么请求触发、需要什么字段、输出什么类型、缺失如何处理、谁决定成功、如何证明没有静默部分成功。

### 23.3 标准实施流程

```text
1. Read：阅读总 Spec、路线图、当前阶段 Spec 和相关代码
2. Baseline：复现现状，保存日志、测试和指标
3. Contract：冻结本阶段 Schema、不变量、错误类型和非目标
4. Test First：先写最小失败测试与关键回归样本
5. Vertical Slice：实现一条最小端到端主链路
6. Failure Injection：验证空结果、超时、过期、冲突和预算耗尽
7. Offline Eval：在冻结集比较 baseline/current
8. Live Gate：用少量真实依赖验证现实字段与集成假设
9. Review：检查业务、架构、安全、可观测性和文档一致性
10. Gate：达到量化门槛后标记阶段完成
11. Handoff：记录结果、限制、遗留问题和下一阶段输入
```

阶段 Plan 应把每一步细化为可独立验证的任务，并写明目标文件、前置依赖、测试命令、预期失败和预期通过结果。实现过程中发现规格错误时应停止相关分支，先修订 Spec 和验收，而不是让实现与文档永久分叉。

### 23.4 规格变更纪律

变更分为三类：

- **澄清**：不改变外部行为，只消除歧义，可直接修订并记录；
- **合同变更**：改变字段、状态、错误或业务范围，必须同步阶段 Spec、测试、Eval 和兼容说明；
- **方向变更**：如旅游转差旅、RAG 技术路线变化，必须先修改总 Spec，再调整路线图和未来阶段。

已通过阶段不会因为新方向自动失效，但必须标记其历史验收的适用范围。M0～M4 因此仍是“通用内核历史完成”；M4.1 建立差旅评测基线，M4.2 修复语义与路由，M4.3 建立新的差旅纵向验收，不伪造过去已经支持该业务。

### 23.5 Agent 协作质量控制

规格可以降低多 Agent 质量参差，但不能单独消除它。需要额外约束：同一阶段只认一个 Schema 来源；子任务具有文件边界和验收命令；实现者不得自行扩大业务范围；Reviewer 使用 Spec 条款而非主观风格判断；集成 Agent 运行统一 Eval；任何未决业务规则保留为 blocker 或 decision record，不能由不同 Agent 各自猜测。

因此，详细规格、测试评估和修复步骤能够有效改善当前问题，但前提是规格包含可否证合同，测试覆盖跨模块连接，真实实验验证外部工具假设，并允许根据证据修改规格。单纯增加文档行数不会自动提高质量。
...
为避免业务调研挤占求职交付时间，首版已经在阶段 Spec 中冻结以下决策：
...
规格驱动开发仍是推荐方法，但其有效性来自“合同可执行、假设可验证、失败可观察、阶段可验收”，而非文档足够长。总 Spec、阶段 Spec、Plan、测试、Offline Eval、Live Gate 和阶段报告共同组成证据闭环；当实验推翻假设时，项目应修订规格，而不是掩盖偏差。
...
- 项目总 Spec：`.codex/rules/架构.md`；
- 总路线图：`.codex/rules/roadmap/00-总路线图.md`；
- 当前阶段：`.codex/rules/roadmap/M4.1-商务差旅评测基线.md`；
- 后续阶段：`.codex/rules/roadmap/M4.2-差旅语义与能力路由.md`、`.codex/rules/roadmap/M4.3-差旅最小纵向切片.md`；
- 项目执行规则：`AGENTS.md`；
