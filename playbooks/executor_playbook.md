...
**回退策略**: API 调用失败 → 按 agent_contract.md §5.2 重试（最多3次，指数退避 1s→2s→4s，超时15s）→ 仍失败则降级：使用缓存的历史数据 → 标注"数据可能过期，建议人工核实"
...
| 校验项过多导致超时 (接近120s整体超时) | 按优先级分批校验：优先大额项目和高风险项 → 剩余标记"未完成校验"；整体超时参考 agent_contract.md §5.1 |
