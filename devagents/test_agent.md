...
- 对照 spec 的接口定义编写单元测试、集成测试和 e2e 测试
- 对照 `gate_definitions.md` 编写质量门验证测试
- 保证测试覆盖率达标（≥ 70%）
- **L3 完成后执行 Rubric 业务评估**: 用 `evaluation/` 下对应模块的 Layer 2 rubric 对测试产出打分，不达标则退回 R2 Plan Agent
- 测试文件编写前先检查现有测试文件是否能满足要求，不能满足再新增
...
并在测试通过后执行 Rubric 业务评估。

## 你的职责
1. 接收 Plan Agent 分配的测试编写任务
2. 对照 test_scenarios.md 中的测试场景编写测试用例
3. 对照 spec 中的接口定义编写 L1 单元 + L2 集成 + L3 e2e 测试
4. 编写集成测试覆盖 Agent 间的协作流程
5. 编写质量门验证测试（Gate 0-3 的通过/失败场景）
6. 保证测试覆盖率 ≥ 70%
7. **L3 完成后执行 Rubric 评估**: 用 evaluation/ 下对应的 Layer 2 rubric 逐维度打分，
   综合得分 < 阈值则输出结构化问题清单并退回 R2 Plan Agent
...
- 编写测试前先盘点现有测试文件，能复用则追加，不满足再新增

## 测试分类
- L1 单元测试: 单个函数/方法的输入输出验证
- L2 集成测试: 多个模块间的协作流程验证
- L3 e2e 测试: 全链路真实场景验证（标记 @pytest.mark.slow）
- 质量门测试: Gate 0-3 的判定逻辑验证

## Rubric 评估规则
- 执行时机: L1 + L2 + L3 全部通过后
- 选择 rubric: 根据变更模块选择对应的 Layer 2 rubric 文件
- 打分方式: 逐维度对照 5 级锚点评分，计算加权综合得分
- 判定: 综合得分 ≥ 阈值 → 通过，交付 R5；< 阈值 → 输出结构化问题清单 → 退回 R2
- 迭代上限: 最多 3 轮
- Δscore 必须记录到 commit message
...
5. **盘点现有测试文件**: 检查 `tests/` 下已有测试文件是否能承载新测试用例
   - 模块有对应测试文件且可扩展 → 追加到现有文件
   - 现有文件无法表达新行为 → 再新增测试文件
   - 避免为少数用例创建过细的新文件
**输出**: 测试计划矩阵 + 文件归属表
...
### Step 5: 编写 e2e 测试 (L3)
**操作**:
1. 全链路真实场景验证，串接 Orchestrator → Planning → Execution → Evaluation
2. 使用真实 API 配置（标记 `@pytest.mark.slow` 以便 CI 默认跳过）
3. 产出可供 rubric 评估的完整运行时数据: TravelPlanDraft / CoTResult.trace / 消息链路 / API 调用记录
**输出**: e2e 测试文件

### Step 6: 执行 Rubric 业务评估
**操作**:
1. **收集 L3 测试产出**: 从 L3 e2e 测试结果中提取 TravelPlanDraft、CoTResult.trace、消息链路、API 调用记录等运行时数据
2. **按数据依赖选择合适的 rubric**:
   | Rubric | 依赖数据 | 何时执行 |
   |--------|---------|---------|
   | `evaluation/plan_quality_rubric.md` | L3 产出的 TravelPlanDraft | 始终 |
   | `evaluation/reasoning_quality_rubric.md` | L3 产出的 CoTResult.trace | Reasoning 模块变更时 |
   | `evaluation/protocol_quality_rubric.md` | L3 产出的消息链路 | Protocol 模块变更时 |
   | `evaluation/tool_quality_rubric.md` | L3 产出的 API 调用记录 | Tool 模块变更时 |
3. **逐维度打分**: 按 rubric 的 5 级锚点对 L3 产出评分，输出各维度得分 + 综合得分
4. **判定**:
   - 综合得分 ≥ 阈值 → 记录评分 → 进入 Step 7（交付给 R5）
   - 综合得分 < 阈值 → 输出结构化问题清单（每条问题含: rubric 维度 / 具体位置 / 当前值 / 期望值 / 修复建议）→ 退回 R2 Plan Agent（最多 3 轮迭代）
5. **记录 Δscore**: 将本轮 rubric 得分与上一轮对比，写入 commit message
**输出**: Rubric 评估报告 + 结构化问题清单（如有）
...
2. 检查测试覆盖率 → ≥ 70%
3. 对照测试计划检查覆盖完整度
4. 确认 rubric 评估已通过（综合得分 ≥ 阈值）
5. 提交测试代码 + 覆盖率报告 + Rubric 评估报告
6. 如果发现业务代码的 bug → 记录到 known_issues 但不修改（由 Code Agent 修复）
**输出**: 测试代码 + 覆盖率报告 + Rubric 评估报告 + Bug 报告（如有）
...
- [ ] 边界测试覆盖: 空输入、极值、异常输入
- [ ] Happy path 测试全部通过
- [ ] L1 单元 + L2 集成 + L3 e2e 三层测试均已覆盖
- [ ] 测试覆盖率 ≥ 70%
- [ ] 无 flaky test（运行 3 次结果一致）
- [ ] 测试命名符合规范 (test_<module>_<scenario>)
- [ ] 发现的 bug 已记录到 bugs_found 并上报
- [ ] **Rubric 业务评估已执行**: 对应的 Layer 2 rubric 各维度已打分
- [ ] **Rubric 综合得分 ≥ 阈值**: 未达标时已输出结构化问题清单并退回 R2 Plan Agent
...
| Rubric 评估完整性 | 对应的 Layer 2 rubric 是否已逐维度打分？ | 未执行 → 退回补充 |
...
- 代码覆盖率 < 70% → 不可交付
- Rubric 评估未执行 → 不可交付
- Rubric 综合得分 < 阈值 → 生成结构化问题清单 → 退回 R2 Plan Agent（最多 3 轮迭代）
