...
from src.obs.trace import trace_session, trace_agent
...
def plan(user_input: str, max_rounds: int = MAX_REVISION_ROUNDS) -> dict:
    """双 LLM 对抗规划。
    ...
    with trace_session("default"):
        ...
        with trace_agent("planner_a", "default"):
            plan_text = ask_llm(build_plan_prompt(user_input, constraints))
        ...
        for round_num in range(max_rounds + 1):
            ...
            # L2: LLM-B 对抗评审
            l2_result = run_l2_review(user_input, plan_text, constraints)
